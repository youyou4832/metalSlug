...
void txtData::txtSaveInt(const char * saveFileName, std::vector<int> vecInt)
{
	char str[128];
	DWORD write;

	strncpy_s(str, 128, vectorArrayCombineInt(vecInt), 126);
...
char * txtData::vectorArrayCombineInt(std::vector<int> vecArray)
{
	char str[128];
	ZeroMemory(str, sizeof(str));
	...
	for (int i = 0; i < vecArray.size(); ++i)
	{
		const char cNum = vecArray[i];
		string strNum = &cNum;

		strncat_s(str, 128, strNum.c_str(), 126);
		if (i + 1 < vecArray.size())
			strcat_s(str, ",");
	}
